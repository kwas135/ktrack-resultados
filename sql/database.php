...
//                        $this->set_charset(UTF_8);
		...
		public function read(){
			...
			$sql = "SELECT numero,nombre,categoria,total,vuelta,lugar FROM (SELECT IF(@categoria='',@categoria:=total,numero)A, numero,nombre,categoria,total,vuelta,
                         IF(categoria!=@categoria,@idx:=1,@idx:=@idx+1)lugar, @categoria:=categoria FROM tsalidas JOIN (SELECT @cateoria:='',@idx=1)T2 WHERE vuelta>0
                         ORDER BY categoria,vuelta desc,total asc )T0  where lugar<=3  ";
			$res = mysqli_query($this->con, $sql);
			return $res;
                        
		}
		public function imprimet(){
			$sql = "SELECT
			numero,nombre,@categoria := categoria AS c,total,vuelta,lugar
			FROM
			(SELECT
			IF(@categoria='',@categoria:=total,numero)A,
			numero,nombre,categoria,total,vuelta,
			IF(categoria!=@categoria,@idx:=1,@idx:=@idx+1)lugar,
			@categoria:=categoria
			FROM tsalidas JOIN (SELECT @cateoria:='',@idx=1)T2 WHERE vuelta>0
			 ORDER BY categoria,vuelta desc,total asc )T0  where lugar<= 5";
			$res = mysqli_query($this->con, $sql);
			return $res;
		...
		public function read3(){
			$sql = "SELECT * FROM tregistro WHERE inicio='si' ORDER BY  numero ASC";
			$res = mysqli_query($this->con, $sql);
			return $res;
		}
			...
			$sql = "SELECT numero, nombre, email, categoria FROM tregistro Order By categoria";
                        $res = mysqli_free_result($this->con, $sql);
			return  $res ;
		}
...
GROUP_CONCAT(finonza.total ORDER BY finonza.vuelta  ASC SEPARATOR '--|--')as TIEMPOS,
